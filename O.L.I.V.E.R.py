    ...
    try:
        ...
        with spr.Microphone() as source:
                            ...
                            if ("open Google" in text):
                                ...
                                if ('is' in text):
                                    text = text.split('remember that ')
                                    text.remove('')
                                    text[0] = text[0].split(' is ')
                                with open('data.dat', 'r')as f1:
                                    a = f1.read()
                                f1.close()
                                if (text[0][0] in a):
                                    converter.say("Sorry Sir, But I can't Change the Data")
                                    converter.runAndWait()
                                else:
                                    converter.say("I will remember that")
                                    converter.runAndWait()
                                    with open('data.dat', 'a+')as f1:
                                        f1.write(text[0][0] + '|' + text[0][1] + '\n')
                            elif(text in a):
                                with open('data.dat', 'r')as f1:
                                    a = f1.read()
                                    f1.seek(0)
                                if (text in a):
                                    i = a.index(text) + len(text)
                                    a = a[i + 1:]
                                    converter.say(a[:a.index('\n')])
                                    converter.runAndWait()
                                else:
                                    converter.say("I don't have any idea about that.")
                                    converter.runAndWait()
                            elif("forget about" in text):
                                with open('data.dat', 'r')as f1:
                                    a = f1.read()
                                    f1.seek(0)
                                a = a.replace(text[13:] + '|', '')
                                ...
                                for i in range(len(b)):
                                    try:
                                        ...
                                        pass
                                with open('data.dat', 'w')as f1:
                                    for i in range(len(b)):
                                        f1.write(b[i] + '\n')
                                converter.say('Data Deleted')
                                converter.runAndWait()
                                continue
